...
通常我们搭建**开发环境**，都会直接安装 **IDE**，即**集成开发环境**。所谓集成开发环境，就是它把开发所需要的各种零碎的东西都打包好了，比如用来编辑代码的**编辑器**、用来把代码翻译成可执行文件的**编译器**、用来调试代码的**调试器**等。IDE 的优点是开箱即用，安装一个 IDE，立马就可以开始开发了，所以很多课程、教学最开始都会推荐新手安装 IDE；IDE 的缺点就是隐藏编译细节和捆绑。

#### 编译环境

而**编译环境**，是开发环境中只负责完成编译过程的部分。

为什么要独立搭建编译环境，而不总是搭建完整的开发环境？因为：

- 我们有时候只需要编译，不需要开发，比如很多开源软件和项目，都支持下载源代码后自己编译；
- 能够更灵活地选择开发环境的各个部分；
- 能够更便捷地编译简单的项目；

### ➥ C/C++ 编译环境的组成

不同的编程语言，从源代码到可执行文件，需要的工具会有所不同，并且往往不止需要一个工具，比如 C/C++ 语言需要**链接器**。因此，我们将从源代码到可执行文件的编译过程中所需要的一系列工具，称为**编译工具链**，更准确些。
